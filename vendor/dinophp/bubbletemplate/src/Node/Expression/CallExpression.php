        ...
        if (\is_string($callable) && false === strpos($callable, '::')) {
            ...
            if ($r instanceof \ReflectionMethod && \is_string($callable[0])) {
                ...
                if (!$compiler->getEnvironment()->hasExtension($class)) {
                    // Compile a non-optimized call to trigger a \Bubble\Error\RuntimeError, which cannot be a compile-time error
    ...
    private function getCallableParameters($callable, bool $isVariadic): array
    {
        ...
        if ($isVariadic) {
            ...
            if ($isArray && $argument->isDefaultValueAvailable() && [] === $argument->getDefaultValue()) {
                ...
                throw new Exception(sprintf('The last parameter of "%s" for %s "%s" must be an array with default value, eg. "array $arg = []".', $callableName, $this->getAttribute('type'), $this->getAttribute('name')));
            }
        }
