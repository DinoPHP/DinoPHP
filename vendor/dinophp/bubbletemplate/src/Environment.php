...
use Bubble\Extension\OptimizerExtension;
use Bubble\Loader\ArrayLoader;
use Bubble\Loader\ChainLoader;
...
class Environment
{
	...
	private $templateClassPrefix = '__BubbleTemplate_';
	 ...
	 *
	 * @param CacheInterface|string|false $cache
	 */
	public function setCache($cache)
	{
		if (\is_string($cache)) {
			...
			throw new Exception('Cache can only be a string, false, or a \Bubble\Cache\CacheInterface implementation.');
		}
	}
	...
	public function loadTemplate(string $cls, string $name, int $index = null): Template
	{
		...
		if (!class_exists($cls, false)) {
			...
			if (!class_exists($cls, false)) {
				...
				if (!class_exists($cls, false)) {
					throw new RuntimeError(sprintf('Failed to load Bubble template "%s", index "%s": cache might be corrupted.', $name, $index), -1, $source);
				}
			}
		}
	 ...
	 *
	 * Similar to load() but it also accepts instances of \Bubble\Template and
	 * \Bubble\TemplateWrapper, and an array of templates where each is tried to be loaded.
	 *
	...
	/**
	 * Returns the runtime implementation of a Bubble element (filter/function/tag/test).
	 *
	...
	public function addFilter(BubbleFilter $filter)
	{
	 ...
	 * @internal
	 */
	public function getFilter(string $name): ?BubbleFilter
	{
	...
	public function addTest(BubbleTest $test)
	{
	 ...
	 * @internal
	 */
	public function getTest(string $name): ?BubbleTest
	{
		return $this->extensionSet->getTest($name);
	}

	public function addFunction(BubbleFunction $function)
	{
	 ...
	 * @internal
	 */
	public function getFunction(string $name): ?BubbleFunction
	{
	...
	public function addGlobal(string $name, $value)
	{
		if ($this->extensionSet->isInitialized() && !\array_key_exists($name, $this->getGlobals())) {
			throw new Exception(sprintf('Unable to add global "%s" as the runtime or the extensions have already been initialized.', $name));
		}
